# Adapted from https://github.com/jsa-aerial/aerobio/blob/master/Scripts/calc_fitness.py - run in python==2.7.17.
...
for eachfile in arguments.fitnessfiles:
	with open(eachfile) as csvfile:
		...
		for line in lines:
			locus = line[9]
			# Null value Set : added by seonhok
			if(locus==''): locus="NONCODE"
			w = line[12]
			if w == 'nW':
				continue
			if not w:
			...
			# prevent program break + convert string to float : modified by seonhok
			try:
				#gene_summary[locus]["w"].append(w) : original file line 181
				gene_summary[locus]["w"].append(float(w))
				gene_summary[locus]["s"].append(avg)
			except:
				print "Error in convert float data", line
...
#This is never called through Galaxy since finding missing genes is just better than not finding them.
	...
	for gene in gene_summary.keys():
		...
		average = sum/num
		xminusxbars = 0
		# dictionary fix : modified by seongok
		#for i in w: original file line 286
		for i in gene_summary[gene]["w"]:
